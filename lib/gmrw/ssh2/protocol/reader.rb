...
class GMRW::SSH2::Protocol::Reader < GMRW::SSH2::Protocol::End
  ...
  rescue SSH2::Protocol::EventError
    notify(/NOT FOUND/, seq_number)
  end
  ...
  def payload
    verify! packet = [
      pack_len    = buffered_read(4).tap{|s| def s.n; to_uint32; end },
      padd_len    = buffered_read(1).tap{|s| def s.n; to_byte  ; end },
